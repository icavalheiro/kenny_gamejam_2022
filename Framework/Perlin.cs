namespace Framework
{
    public class Perlin
    {
        ...
        public int repeat { get; }                                              // Doubled permutation to avoid overflow
        ...
        public double GetOctavePerlin(double x, double y, double z, int octaves, double persistence)
        {
            double total = 0;
            double frequency = 1;
            double amplitude = 1;
        ...
        public double GetPerlin(double x, double y, double z)
        {
            ...
            double x1, x2, y1, y2;
            x1 = Math.Lerp(Grad(p[aa], xf, yf, zf),          // This is where the "magic" happens.  We calculate a new set of p[] values and use that to get
            ...
            x2 = Math.Lerp(Grad(p[ab], xf, yf - 1, zf),          // we interpolate between the y-values to get a z-value.
                        Grad(p[bb], xf - 1, yf - 1, zf),
                        u);                                     // When calculating the p[] values, remember that above, p[a+1] expands to p[xi]+yi+1 -- so you are
            y1 = Math.Lerp(x1, x2, v);                               // essentially adding 1 to yi.  Likewise, p[ab+1] expands to p[p[xi]+yi+1]+zi+1] -- so you are adding
                                                                     // to zi.  The other 3 parameters are your possible return values (see grad()), which are actually
            x1 = Math.Lerp(Grad(p[aa + 1], xf, yf, zf - 1),      // the vectors from the edges of the unit cube to the point in the unit cube itself.
                        Grad(p[ba + 1], xf - 1, yf, zf - 1),
                        u);
            x2 = Math.Lerp(Grad(p[ab + 1], xf, yf - 1, zf - 1),
                          Grad(p[bb + 1], xf - 1, yf - 1, zf - 1),
                          u);
            y2 = Math.Lerp(x1, x2, v);

            return (Math.Lerp(y1, y2, w) + 1) / 2;                       // For convenience we bound it to 0 - 1 (theoretical min/max before is -1 - 1)
        }
